...
export function initializeDashboard() {
  if (document.getElementById('income-display')) {
    ...
    // month independently of Transactions/Reports, so if it's still
    // showing a past month, defaulting this field to today's real date
    // silently filed new transactions under that stale month while the
    // date field looked correct — the transaction then never showed up
...
function bindEvents() {
  ...
  if (expenseCategoryInput && expenseAmountInput && expenseLimitWarning) {
    const updateWarning = () => {
      const p = getPeriodData(currentPeriod);
      renderCategoryLimitWarning(expenseLimitWarning, p.expenses, expenseCategoryInput.value, Number(expenseAmountInput.value));
    };
    expenseCategoryInput.addEventListener("change", updateWarning);
    expenseAmountInput.addEventListener("input", updateWarning);
  }

  // Same live warning for the Edit Expense modal, excluding the
  // transaction currently being edited from its own "already spent" total.
  ...
  if (editExpenseCategoryInput && editExpenseAmountInput && editExpenseLimitWarning) {
    const updateEditWarning = () => {
      const p = getPeriodData(currentPeriod);
      renderCategoryLimitWarning(editExpenseLimitWarning, p.expenses, editExpenseCategoryInput.value, Number(editExpenseAmountInput.value), editingExpenseIndex);
    };
    editExpenseCategoryInput.addEventListener("change", updateEditWarning);
  ...
  // editingExpenseIndex/editingIncomeIndex variables in THIS file, which
  // are never the same binding as the window properties. Reset the real
  // variables here too, or closing without saving leaves the "currently
  // editing" guard silently pointing at the last-opened index.
  const closeEditExpenseBtn = document.getElementById("close-edit-expense");
  if (closeEditExpenseBtn) {
    closeEditExpenseBtn.addEventListener("click", () => {
  ...
  if (closeEditIncomeBtn) {
    closeEditIncomeBtn.addEventListener("click", () => {
