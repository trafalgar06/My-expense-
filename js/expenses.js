...
function bindEvents() {
  ...
  if (clearMonthBtn) {
    clearMonthBtn.addEventListener("click", () => {
      ...
      // each transaction's effect on its account — leaves those running
      // balances permanently wrong (there's no "reconcile" for bank or
...
function openAddExpenseModal() {
  ...
  if (expenseCategorySelect && expenseAmountInput && expenseLimitWarning) {
    const updateWarning = () => {
      const p = getPeriodData(currentPeriod);
      renderCategoryLimitWarning(expenseLimitWarning, p.expenses, expenseCategorySelect.value, Number(expenseAmountInput.value));
    };
    expenseCategorySelect.addEventListener("change", updateWarning);
...
function addExpense() {
  ...
  // Writing the template first meant that call found the brand-new rule,
  // generated its first occurrence, and then the manual push below added
  // the very same transaction again — so ticking "Recurring" silently
  // created two identical entries and debited the account twice. Saving
  // the template last lets the generator's duplicate check see the
  // existing entry and skip it.
  if (pendingTemplate) {
    const templates = safeJSONParse("recurringTransactions", []);
    templates.push(pendingTemplate);
...
function renderExpenses() {
  ...
  // are computed (adjustAccountBalance calls, list row icons, etc.).
  if (expensesFilters.account && expensesFilters.account !== "all") {
  ...
  all.forEach((t) => {
    ...
    row.innerHTML = `
      <div class="expense-details">
        <span class="tx-icon-badge" style="background:${badgeColor}">${escapeHtml(icon)}</span>
        <div class="text-body font-medium">
          <span class="tx-name-row">${escapeHtml(name)}${isRecurring ? ' 🔁' : ''} <span title="${accountLabel}" style="font-size: 0.85em;">${accountIcon}</span></span>
  ...
  document.querySelectorAll(".delete-btn").forEach(button => {
    button.addEventListener("click", function () {
      ...
      // A generated recurring entry would be recreated by the next
      // ensurePeriod() call, so deleting one had no visible effect at all.
      // Record the deletion, and — since nothing else in the app can stop
      // a recurring rule once created — offer to end the whole series.
      if (target.recurringTemplateId) {
        const stopSeries = confirm(
          "This is a recurring transaction.\n\n" +
          "OK — stop it repeating from now on (deletes the recurring rule).\n" +
          "Cancel — delete only this one entry and keep the rest."
        );
        if (stopSeries) {
          removeRecurringTemplate(target.recurringTemplateId);
        } else {
          addRecurringSkip(target.recurringTemplateId, target.date);
        }
      }
...
export function initializeExpensesPage() {
  if (document.getElementById('expense-list')) {
    ...
    if (sessionStorage.getItem("pendingAction") === "add-expense") {
